...
def _row(e):
    ...
    return [
        ...
        -round(e.penalty) if e.penalty_on and e.penalty else "",
...
def export_xlsx(path, title, days, meta=None):
    ...
    def flush_week():
        ...
        vals = ["", "", "Итого за неделю", fmt_hm_short(t.work_min), "",
                ...
                round(t.day_pay), round(t.extra_pay), round(t.bonus), -round(t.penalty), round(t.total_pay)]
...
def export_pdf(path, title, days, meta=None):
    ...
    def flush_week():
        ...
        data.append(["", "", "Итого за неделю", fmt_hm_short(t.work_min), "",
                     ...
                     "%d" % round(t.bonus), "%d" % round(t.penalty), "%d" % round(t.total_pay)])
    ...
    widths = [17, 10, 30, 17, 26, 15, 88, 22, 21, 16, 16, 24]
    total_w = sum(widths)
...
def export_text(days, title, meta=None):
    ...
    for e in days:
        ...
        s += " | штраф: -%d р." % round(e.penalty) if e.penalty_on and e.penalty else ""
